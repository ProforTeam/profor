...
%
% * 'GridLinesSpace': Integer for spacing between gridlines. Added by lat.
%   Default=[], i.e. gridlines for each tick. 
%
...
p.addParamValue('GridLinesSpace',1);
p.parse(varargin{:});
...
if ~strcmp(p.GridLines,'none')
    ...
    for i = p.GridLinesSpace:p.GridLinesSpace:diff(xlim)-1
        line('Parent',p.hAxes,'XData',[i i]+.5, 'YData', ylim, 'LineStyle', p.GridLines);
    end
    for i = p.GridLinesSpace:p.GridLinesSpace:diff(ylim)-1
