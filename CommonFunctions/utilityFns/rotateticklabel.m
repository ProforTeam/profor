...
if rot<180
    if ~isempty(get(h,'zticklabel'))
        d=get(h,'ZTick');
        th=text(b,repmat(c(1)-.1*(c(2)-c(1)),length(b),1),repmat(d(1)-.1*(d(2)-d(1)),length(b),1),a,'HorizontalAlignment','right','rotation',rot);
%     elseif ~isempty(get(h,'ZTick'));
%         d=get(h,'ZTick');
%         %th=text(b,zeros(length(b),1),a,'HorizontalAlignment','right','rotation',rot);
%         th=text(b,repmat(d(1)-.1*(d(2)-d(1)),length(b),1),a,'HorizontalAlignment','right','rotation',rot);
    else
