function[fillhandle,msg]=jbfill(xpoints,upper,lower,color,edge,add,transparency,edge_linestyle)
...
%     grid on
%     legend('Datr')
if nargin<8;edge_linestyle='-';end;% default style of edge
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    set(fillhandle,'EdgeColor',edge,'FaceAlpha',transparency,'EdgeAlpha',transparency,'LineStyle',edge_linestyle);%set edge color
    if add
        hold off
    end
else
