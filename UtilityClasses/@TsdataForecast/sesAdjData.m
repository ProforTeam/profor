...
%
% See also DATASETTING, TSDATA.SESADJDATA
%
...
for j = 1 : nObj
    if strcmpi( obj(j).dataSettings.doSesAdj, 'y' )
        ...
        sesAdjustedDataPoint   = nbX12arima(dataPoint, options);
        sesAdjustedDataSim     = nbX12arima(forecastSimulations, options);

        % Extract correct information        
        dataPoint               = sesAdjustedDataPoint.sesadj{1};
        forecastSimulations     = sesAdjustedDataSim.sesadj{1}(end - numberOfForecast + 1:end,:);                    
        forecastDates           = dataDates(end - numberOfForecast + 1:end,:);                        
                
        obj(j).ts               = TsdataForecast.setTs(dataDates, dataPoint);                
